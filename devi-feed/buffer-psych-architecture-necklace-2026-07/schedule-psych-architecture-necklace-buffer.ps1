# Queue Gold Stays psych-architecture necklace reel to Buffer (Devi IG).
#
# powershell -ExecutionPolicy Bypass -File devi-feed/buffer-psych-architecture-necklace-2026-07/schedule-psych-architecture-necklace-buffer.ps1
# powershell -ExecutionPolicy Bypass -File devi-feed/buffer-psych-architecture-necklace-2026-07/schedule-psych-architecture-necklace-buffer.ps1 -DryRun
...
$manifestPath = Join-Path $packDir "schedule-buffer-psych-architecture-necklace.json"
...
$reelFile = "necklace-psych-architecture-gold-stays-reel.mp4"
$deliveryReel = Join-Path $repoRoot "buffer-delivery\$reelFile"
$sourceReel = Join-Path $repoRoot "buffer-delivery\necklace-psych-architecture-4k-campaign-v2-signed-reel.mp4"

if (-not (Test-Path $sourceReel)) { throw "Missing signed source reel: $sourceReel" }
