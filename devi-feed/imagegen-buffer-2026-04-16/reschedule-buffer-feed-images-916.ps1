# Reschedule Buffer IG+FB *feed image* posts from feed-4x5 URLs back to full 9:16 Remotion overlays.
# Run from repo root: pwsh -File devi-feed/imagegen-buffer-2026-04-16/reschedule-buffer-feed-images-916.ps1
# Requires: scripts/buffer-common.ps1 (local), local-secrets/buffer_access_token.txt
# Note: Instagram feed via API may reject 9:16 images; if createPost fails, use 4:5 assets or Reel/video instead.
...
$raw = "https://raw.githubusercontent.com/ortall0201/Social/main/devi-feed/imagegen-buffer-2026-04-16/with-caption"
...
function Invoke-ScheduledImages($channelId, $serviceName) {
  ...
  foreach ($e in $edges) {
    ...
    if (-not $src -or $src -notmatch "feed-4x5/devi-buffer-card-(\d+)-with-caption-feed-4x5\.png") { continue }
    $nn = "{0:D2}" -f [int]$matches[1]
    $newUrl = "$raw/devi-buffer-card-$nn-with-caption.png"
    Write-Host "[$serviceName] $($n.id) -> 9:16 card $nn"
