#!/usr/bin/env node
/**
 * Renders 1:1 (1080×1080) stills: full frame via object-fit contain + editorial type.
 * Same slide list as 9:16 overlay; output filenames *-with-caption-1x1.png.
 *
...
const outDir = path.join(remotionRoot, "out/devi-feed-caption-overlays-1x1");
...
console.log("Bundling Remotion (1:1)…");
...
for (const s of slides) {
  ...
  const baseName = s.file.replace(/\.png$/i, "-with-caption-1x1.png");
  ...
  JSON.stringify(
    {
      ...
      compositionId: "DeviFeedImageCaptionSquare",
      format: "1080x1080",
      slides: rendered,
    },
    null,
