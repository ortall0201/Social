#!/usr/bin/env node
/**
 * Renders 9:16 stills: ImageGen cards + Devi editorial type (headline + niche subline).
 * Copies source PNGs from repo `devi-feed/imagegen-buffer-2026-04-16/` into `public/devi-feed-buffer/`.
 *
...
const srcDir = path.join(
  repoRoot,
  "devi-feed/imagegen-buffer-2026-04-16"
);
const pubDir = path.join(remotionRoot, "public/devi-feed-buffer");
const outDir = path.join(remotionRoot, "out/devi-feed-caption-overlays");

/** Viraly headline + Devi-voice subline (fashion-first, moment-specific). */
const slides = [
  {
...
for (const s of slides) {
  ...
  const baseName = s.file.replace(/\.png$/i, "-with-caption.png");
  ...
  JSON.stringify(
    {
      ...
      compositionId: "DeviFeedImageCaption",
      format: "1080x1920",
      slides: rendered,
    },
    null,
...
console.log(`Done. ${rendered.length} stills in ${elapsed}s. Manifest: ${manifestPath}`);
